    ...
    for t in np.arange(delta_t * 2, delta_t * iteracoes, delta_t):
        ...
        for x in np.arange(intervalo[0], intervalo[1]+delta_x, delta_x):
            if x == intervalo[0]:
                ...
                u_i_k_mais_um = gama_quadrado * (u[k][i-1] - 2*u[k][i] + u[k][i+1]) - u[k-1][i]
...
print(diferencas_finitas_exp(4, 0.01, 0.2, [0, 1], x_vezez_x_menos_1, [sen_pi_vezes_t, zero], dois, 10))
